...
class GpuSimulator(BaseSimulator):
    def __init__(self, n, verbose=False):
        ...
        """
        super.__init__()
        self.basis_gates += ["xrot", "yrot", "zrot", "xxrot"]
        self.n = n
        self.dim = 2 ** n
    ...
    def apply(self, gate, ind1, ind2=None, ind3=None, param=None, update=True):
        actual_gate_representation = gate
        if gate in self.mapper.keys():
            actual_gate_representation = self.mapper[gate]
        self.apply_impl(actual_gate_representation, ind1, ind2=ind2, ind3=ind3, theta=param, update=update)
        pass

    def apply_impl(self, gate, ind1, ind2=None, ind3=None, theta=None, update=True):
        """
        ...
        if (gate in qcgate.oneGate):
            ...
            if (self.verbose): print(
                "Apply threequbit gate {} to ({},{},{})-th qubit".format(gateName, ind1, ind2, ind3))
            ...
            if (self.verbose): print(
                "Generic unitary ({},{},{}) on {}-th qubit with {}".format(theta[0], theta[1], theta[2], ind1,
                                                                           gateName))
            self.currentTrace = None
        else:
